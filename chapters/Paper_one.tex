...
$h(x,t)$~\cite{ReynoldsLubr,oronLongscaleEvolutionThin1997,RevModPhys.81.1131,Mitlin}:
...
layer at rest~\cite{PhysRevE.65.036309}.
...
certain liquid and can be calculated using Young's equation \cite{RevModPhys.57.827}
...
is required, as the one in Eqs. (\ref{eq:derivative}-\ref{eq:n_laplace})~\cite{THAMPI20131}. 
In particular, we note that the isotropy of the pressure gradient is of utmost importance: a simple scheme with two-point centered derivatives~\cite{zhou2004lattice} yields squared equilibrium droplet shapes.
