...
This effect can effectively be used in so-called fog harvesting devices, where ambient water vapor is condensed and collected in structures which promote droplet coarsening~\cite{zhangInkjetPrintingDirect2015, shiFogHarvestingHarps2018}.
...
More generally, droplet coalescence has attracted much attention in the past two decades, see refs.~\cite{eggersCoalescenceLiquidDrops1999, duchemin_eggers_josserand_2003, aartsHydrodynamicsDropletCoalescence2005, PhysRevLett.106.114501, doi:10.1063/1.4828721}. 
...
The theoretical approach we use to study this system is the lubrication approximation~\cite{Reynolds, oronLongscaleEvolutionThin1997, diezGlobalModelsMoving2000}.
Applying this approximation to the Navier-Stokes equation yields the thin film equation, which for a singular horizontal dimension reads~\cite{bonnWettingSpreading2009, crasterDynamicsStabilityThin2009, thielePatternedDepositionMoving2014}
...
The pressure accounts for both the surface tension and the correct fluid substrate behavior (wetting)~\cite{zitzLatticeBoltzmannMethod2019}.
We therefore write the pressure as
\begin{equation}\label{eq:pressure_five}
    p = \gamma \partial_x^2 h + \Pi(h),
\end{equation}
where the first term is the (1D) Laplacian of the liquid-vapor interface and $\Pi(h)$ is the disjoining pressure~\cite{oronLongscaleEvolutionThin1997, bonnWettingSpreading2009, peschkaSignaturesSlipDewetting2019, diezGlobalModelsMoving2000},
...
This prefactor can be understood as a linking factor to the Hamaker constant ($\mathcal{A}$) with~~\cite{diezMetallicthinfilmInstabilitySpatially2016, bestehorn20033d, van1988interfacial}
\begin{equation}
...
For an in deep presentation of the derivation we refer to the work by Schwartz and Eley and Bonn et al.~\cite{schwartzSimulationDropletMotion1998, bonnWettingSpreading2009}.
...
Following the argumentation of Riegler, Lazar and Eddie et al.~\cite{doi:10.1021/la800630w, eddiInfluenceDropletGeometry2013},
...
Upon rescaling, Eddi et al. found in their analytical analysis that $h_0$ is the only relevant scale, justifying therefore $\kappa \sim 1/h_0$~\cite{eddiInfluenceDropletGeometry2013}. 
...
\begin{equation}\label{eq:coal_powerlaw}
    h_0(t) \sim t^{2/3},
\end{equation}
for contact angles below $\theta < \pi/2$~\cite{eddiInfluenceDropletGeometry2013, keller2002breaking}.
...
Performing the limits discussed in ref.~\cite{zitzLatticeBoltzmannMethod2019, zitzLatticeBoltzmannSimulations2021}, this system becomes an effective solver for Eq~(\ref{eq:thin_with_marangoni}).
...
The film thickness $h$ and the velocity at the free surface $u$ are moments of the distribution functions $f_i$~\cite{salmonLatticeBoltzmannMethod1999, dellarNonhydrodynamicModesPriori2002, zitzLatticeBoltzmannSimulations2021}:
