...
% In a time dependent problem, which the thin film equation is, we have to be aware that a discretization in space may be very different from a discretization in time.
% Often a good approach is to first solve the homogenous problem and only than advance in time.
...
However all finite difference schemes suffer from the concrete choice of the discretization, which is often referred as discretization artifacts.
...
Thinking about a wave package, numerically it should be ensured that several time iterations are computed before the package has reached the next spatial point of the grid.
...
However, when simulating the thin film equation we may be interested in forces, pressures or simply the thickness $h(\mathbf{x},t)$, which is why we need a backward transformation.
...
\subsection{Shallow water equations on the lattice}\label{susbsec:shallow_water_lattice_boltzmann}
...
In Chap.~\ref{chapter:first_paper} we will recap the main the ideas of this derivation. 
